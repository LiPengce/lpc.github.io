...
fn d_open_internal<E: Pairing>(
    ...
    let mut q = vec![E::ScalarField::zero(); 1 << (sub_nv - 1)];
    let mut r = vec![E::ScalarField::zero(); 1 << (sub_nv - 1)];
    ...
    if Net::am_master() {
        ...
        let mut q = vec![E::ScalarField::zero(); 1 << (m - 1)];
        let mut r = vec![E::ScalarField::zero(); 1 << (m - 1)];

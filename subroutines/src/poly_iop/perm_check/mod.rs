    ...
    F: PrimeField,
{
    ...
    fn d_prove(
        ...
        perms: &[Self::MultilinearExtension],
        transcript: &mut IOPTranscript<F>,
    ) -> Result<Option<(Self::PermutationProof, Vec<Vec<F>>)>, PolyIOPErrors>;
    ...
    F: PrimeField,
{
    ...
    fn prove(
        ...
        let mut leaves = compute_leaves::<F, false>(&beta, &gamma, fxs, gxs, perms)?;
    ...
    fn d_prove(
        ...
        let mut leaves = compute_leaves::<F, true>(&beta, &gamma, fxs, gxs, perms)?;
        ...
        let mut to_prove = leaves
            ...
            .map(|mut leave| {
                ...
                let (batched_circuit, companion_circuit) =
                    <BatchedDenseGrandProduct<F> as BatchedGrandProduct<F>>::d_construct(take(
                        &mut leave,
                    ));
                if Net::am_master() {
        ...
        for (batched_circuit, companion_circuit, f_claims, g_claims) in to_prove.iter_mut() {
            let proof_ret =
                <BatchedDenseGrandProduct<F> as BatchedGrandProduct<F>>::d_prove_grand_product(
                    batched_circuit,

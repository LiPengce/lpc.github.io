use crate::poly_iop::{
    sum_check::{
        ...
        generic_sumcheck::SumcheckInstanceProof,
    },
    utils::drop_in_background_thread,
};
use arithmetic::{bind_poly_var_bot, eq_poly::EqPolynomial, math::Math, Fraction, OptimizedMul};
...
pub trait BatchedRationalSumLayer<F: PrimeField>: BatchedCubicSumcheckInstance<F> {
    ...
    fn prove_layer(
        ...
        let mut eq_poly = DenseMultilinearExtension::from_evaluations_vec(
            r_rational_sum.len(),
            EqPolynomial::<F>::evals(r_rational_sum),
        );
    ...
    fn d_prove_layer(
        ...
        let mut eq_poly = DenseMultilinearExtension::from_evaluations_vec(
            r_rational_sum.len() - num_party_vars,
            EqPolynomial::<F>::evals(&r_rational_sum[num_party_vars..]),
        );

        let proof = self.d_prove_sumcheck(
            &claim,
            &coeffs,
            &mut eq_poly,
            &r_rational_sum[..num_party_vars],
            transcript,
            &lambda_layer,
        );
...
fn compute_cubic_direct<F: PrimeField>(
    ...
    let eq_poly = &evaluations[4][0];

    (0..eq_poly.evaluations.len() / 2)
        .into_par_iter()
        .map(|i| {
            let eq_evals = {
            ...
            .for_each(|(batch_index, (left_p, left_q, right_p, right_q))| {
                ...
                evals.2 += coeffs[batch_index]
                    * (right_eval_3.p * left_eval_3.q
                        + (left_eval_3.p + *lambda * left_eval_3.q) * right_eval_3.q);
            });
    ...
    for BatchedDenseRationalSumLayer<F, C>
{
        ...
        rayon::scope(|s| {
            s.spawn(|_| {
                self.layers_p.par_iter_mut().for_each(|layer: &mut Vec<F>| {
                    let mut new_layer = vec![F::zero(); self.layer_len / 2];
                    new_layer
    ...
    fn compute_cubic_direct(
        ...
        compute_cubic_direct(coeffs, evaluations, lambda)
    }
}
                    ...
                    .map(|(layer, coeff)| match layer {
                        ...
                        DynamicDensityRationalSumLayer::Sparse(sparse_layer) => {
                            ...
                            for (j, (index, value)) in sparse_layer.iter().enumerate() {
                                ...
                                sum.0 +=
                                    eq_eval_0.mul_0_optimized(inner_func_delta(left.0, right.0));
                                sum.1 += eq_eval_2
                            ...
                            (*coeff * sum.0, *coeff * sum.1, *coeff * sum.2)
                        },
                        ...
                        DynamicDensityRationalSumLayer::Dense(dense_layer) => {
                            ...
                            (*coeff * (evals.0 + default_sum.0), *coeff * (evals.1 + default_sum.1), *coeff * (evals.2 + default_sum.2))
                        },
                    })
                    .reduce(
                        || (F::zero(), F::zero(), F::zero()),

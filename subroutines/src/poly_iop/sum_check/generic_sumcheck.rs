...
use arithmetic::{
    bind_poly_var_bot, math::Math, unipoly::{CompressedUniPoly, UniPoly}
};
...
impl<F: PrimeField> SumcheckInstanceProof<F> {
        ...
        Func: Fn(&[F]) -> F + std::marker::Sync,
    {
        ...
        for _round in 0..num_rounds {
            ...
            polys
                .par_iter_mut()
                .for_each(|poly| bind_poly_var_bot(Arc::get_mut(poly).unwrap(), &r_j));
        ...
        Func: Fn(&[F]) -> F + std::marker::Sync,
    {
        ...
        for _round in 0..num_rounds {
            ...
            let r_j = if Net::am_master() {
                ...
                let eval_points = all_eval_points
                    .iter()

...
fn test_sumcheck(
    ...
    let proof = <PolyIOP<Fr> as SumCheck<Fr>>::d_prove(&poly, &mut transcript)?;
...
fn test_generic_sumcheck(
    ...
    let mut polys = 
        ...
        .take(NUM_POLYS)
        .collect::<Vec<_>>();
    let polys_clone = polys.iter().map(
        |poly| Arc::new(DenseMultilinearExtension::clone(poly))
    ).collect::<Vec<_>>();

    let combine_func = |evals: &[Fr]| {
        evals[0] * evals[1] + evals[2] * evals[3] * evals[4]
    };
    
    let claim = (0..polys[0].evaluations.len())
        .map(|eval_index| {
            let evals = polys.iter().map(|poly| poly.evaluations[eval_index]).collect::<Vec<_>>();
            combine_func(&evals)
        })
        .sum::<Fr>();
    let all_claims = Net::send_to_master(&claim);

    let proof_result = SumcheckInstanceProof::d_prove_arbitrary(&Fr::zero(), nv, &mut polys, combine_func, 3, &mut transcript);
    ...
    if Net::am_master() {
        ...
        let (claimed_eval, r_verifier) = proof.verify(all_claims.iter().sum(), nv + num_party_vars, 3, &mut transcript).unwrap();

        assert_eq!(r_prover, r_verifier);

        let evals = polys_clone.iter()

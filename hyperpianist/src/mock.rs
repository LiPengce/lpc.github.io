...
impl<F: PrimeField> MockCircuit<F> {
    ...
    pub fn d_new<R: RngCore>(num_constraints: usize, gate: &CustomizedGates, rng: &mut R) -> MockCircuit<F> {
        let nv = log2(num_constraints);
        let num_selectors = gate.num_selector_columns();
        let num_witnesses = gate.num_witness_columns();
        let log_n_wires = log2(num_witnesses);
        ...
        let mut permutation = Vec::with_capacity(num_witnesses * num_constraints);
        let mut offset = 0;
        for _ in 0..num_witnesses {
            permutation.extend(
                (Net::party_id() * num_constraints..(Net::party_id() + 1) * num_constraints)
                    .map(|i| F::from((i + offset) as u64)),
            );

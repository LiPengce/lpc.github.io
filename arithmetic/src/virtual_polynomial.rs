...
use crate::{eq_poly::EqPolynomial, errors::ArithErrors, multilinear_polynomial::random_zero_mle_list, random_mle_list};
...
impl<F: PrimeField> VirtualPolynomial<F> {
    ...
    pub fn replace_mles(
        &mut self,
        new_mle_list: Vec<Arc<DenseMultilinearExtension<F>>>,
    )  {
    ...
    pub fn mul_by_mle(
        ...
        if self.flattened_ml_extensions.len() != 0 && mle.num_vars != self.flattened_ml_extensions[0].num_vars {
            return Err(ArithErrors::InvalidParameters(format!(
                "product has a multiplicand with wrong number of variables {} vs {}",
...
/// over r, which is
///      eq(x,y) = \prod_i=1^num_var (x_i * r_i + (1-x_i)*(1-r_i))
pub fn build_eq_x_r_vec_with_coeff<F: PrimeField>(r: &[F], coeff: &F) -> Result<Vec<F>, ArithErrors> {
    // we build eq(x,r) from its evaluations
    // we want to evaluate eq(x,r) over x \in {0, 1}^num_vars
    // for example, with num_vars = 4, x is a binary vector of 4, then
    //  0 0 0 0 -> (1-r0)   * (1-r1)    * (1-r2)    * (1-r3)
...
/// This function takes `r.len()` steps, and for each step it requires a maximum
/// `r.len()-1` multiplications.
fn build_eq_x_r_helper_with_coeff<F: PrimeField>(r: &[F], coeff: &F, buf: &mut Vec<F>) -> Result<(), ArithErrors> {
    if r.is_empty() {
        return Err(ArithErrors::InvalidParameters("r length is 0".to_string()));
    } else if r.len() == 1 {
        // initializing the buffer with [1-r_0, r_0]
